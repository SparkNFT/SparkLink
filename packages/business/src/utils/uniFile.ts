...
export class UniFile implements IUniFile {
  ...
  constructor(
    environmentSpecificFile:
      | File
      | { arrayBuffer: ArrayBuffer; name: string; type: string }
      | { buffer: NodeBuffer; name: string; path: string; type: string }
  ) {
    if (environmentSpecificFile["arrayBuffer"] instanceof ArrayBuffer) {
      this.arrayBufferPromise = new Promise((resolve) =>
        resolve(environmentSpecificFile["arrayBuffer"])
      );
      this.type = environmentSpecificFile.type;
      this.name = environmentSpecificFile.name;
      this.path = environmentSpecificFile["path"];
    } else if (environmentSpecificFile["arrayBuffer"]) {
