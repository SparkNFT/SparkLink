		...
		<div class="detail-container">
			...
			<div class="info">
				...
				<div class="ntf-detail">
					<p>{{ t("detail.price", {price: sellingPriceInCoin, symbol: token.symbol}) }}</p>
					<p>
						{{t("detail.royalty", {price: toCoin(metadata.royaltyPrice), symbol: token.symbol})}}
					</p>
					<p>{{ t("detail.max_shares_number", {times: metadata.maxShareTimes}) }}</p>
					<p>
						{{t("detail.percentage_earning", {percentage: metadata.percentageOfEarnings})}}
					</p>
					<p>{{ t("detail.work_is_encrypted", {encrypted: metadata.encrypted}) }}</p>
					<p>
						{{t("detail.allow_secondary_creation", {allow: metadata.allowSecondaryCreation})}}
					</p>
					<p>{{ t("detail.dividend_ratio", {percentage: metadata.percentageOfEarnings}) }}</p>
					<p>{{ t("detail.child_node_owned", {}) }}</p>
					<p>{{ t("detail.nft_is_encrypted", {encrypted: metadata.encrypted}) }}</p>
				</div>
				<div>
					<button class="btn white" @click="copyLink">{{ t("share.link") }}</button>
...
const {t} = useI18n({
	messages: {
		en: {
			...
			notes: "💡Everyone can view and purchase this work by sharing links or posters to the details page of the work."
		},
		"zh-CN": {
			title: "Collection",
			current_earning: "Current Earnings",
...
const factory = computed(() => store.getters["web3/userOperatorFactory"] as UserOperatorFactory);
...
const isCurrentChain = computed(() => chainId.value === web3InfoGetter.chain.id.value);
...
function setDownloader(emitter: DownloadEventEmitter, _downloader: IContentDownloader) {
