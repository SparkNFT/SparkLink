	...
	>
		...
		<template #bottom>
			<el-button
				type="primary"
				:disabled="!canClose"
				@click="router.push({name: 'art',params: {chainId: web3InfoGetter.chain.id.value, nftId}})"
			>
				View
			</el-button>
			<el-button
...
<script lang="ts" setup>
		...
		messages: {
			en: {
				...
				steps: {
					...
					publish: {before: "Publish", doing: "Publishing", after: "Published", confirm: "Confirm num: "},
					signRequest: {
						before: "Sign request",
			...
			"zh-CN": {
				...
				steps: {
					...
					publish: {before: "发布", doing: "正在发布", after: "已发布", confirm: "确认数："},
					signRequest: {
						before: "签署请求",
						doing: "正在签署请求",
						after: "请求已签署",
	...
	props.eventEmitter.on("publishConfirmation", (confirm: number) => {
		customMessage.value = {
			...
			message: `${t("steps.publish.confirm")}[${confirm}/${minConfirmationNum.value}]`,
		};
	});
