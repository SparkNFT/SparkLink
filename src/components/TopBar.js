...
class TopBar extends Component {
	...
	async componentDidMount() {
		let onWindowHrefChange = ()=>{
			...
			if(hash == '#/publish'|| hash.indexOf('publish')>0){
				newHash = 'publish'
			}
			if(hash=='#/collections'){
				newHash = 'collection'
		...
		return (
			<div>
				...
				>
					...
					<MenuItem onClick={handleMenuClose}>
						<Button size="medium" style={{ width: '100%' }} className={classes.btnItem} href="/#/Publish">
							{t('publish')}
						</Button>
					</MenuItem>
					<MenuItem onClick={handleMenuClose}>
				...
				<Toolbar id='topbar' className={classes.noPadding} style={{ minHeight: '0px' }}>
					...
					<Grid style={fixStyle} className={classes.Toolbar + ' ' + classes.PaddingT11 + ' ' + classes.PaddingB11} container direction="row" wrap="nowrap">
						<Grid item className={classes.titleGrid}>
							<IconButton style={menuStyle} className={classes.btnLan+' '+classes.MarginL10} aria-controls="basic-menu" aria-haspopup="true" aria-expanded={open ? 'true' : undefined} onClick={handleMenuClick}>
							...
							<div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', height: '100%' ,flex:'1',alignItems:'center'}}>
								<div className={classes.btngroup}>
									...
									<a size="medium" className={classes.btnTopbar+' '+((this.state.nowHash=='publish')&&classes.btnTopbarSelect)} href="/#/Publish">
										{t('publish')}
									</a>
