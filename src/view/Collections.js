...
const styles = (theme) => ({
	...
	card: {
		...
		background: '#E9DEC7',
		borderRadius: '10px',
		filter: 'drop-shadow(3px 4px 6px rgba(140, 140, 140, 0.15))',
		boxShadow:'none',
	...
	async componentDidMount() {
		...
		for (let i = 0; i < ids.length; i++) {
			...
			if(metadatas[i].image == 'No Set'){
				element['description'] = t('no_upload_file')
	...
	getMetadata = async (contract, ids) => {
		return Promise.all(ids.map(async (id) => {
			...
			while (error_count <= 2) {
				...
				try {
					if (ipfs_hash === 'QmNLei78zWmzUdbeRB3CiUfAizWUrbeeZh5K1rhAQKCh51' || ipfs_hash === 'QmWES6p1Sm9bgfvPCAunBnsSsQBhVuqUrqAPnmrYvWSFfx') {
						return {
							'name': t('Untitled'),
							'description': t('no_upload_file'),
							'image': 'No Set',
							'attributes': [
								{
						...
						<li>
							<Card className={classes.card}>
									...
									</Typography>
									{
										(card.description.length > 50)?(
											<Tooltip title={card.description}>
												<Typography className={classes.Display12s} style={{ color: 'black',fontWeight:'300',display:'-webkit-box',WebkitBoxOrient:'vertical',WebkitLineClamp:3,overflow:'hidden' }}  >
													{card.description}
												</Typography>
											</Tooltip>
										):(
								...
								<CardActions style={{display:'flex',justifyContent:'space-between',marginTop:'5%',paddingLeft:'5%',paddingBottom:'5%'}}>
									...
									<Button className={classes.btnMini} style={{display:(card.image == 'No Set')?(''):('none')}} href={'/#/Publish/' + card.id}>
										{t('去上传')}
									</Button>
							...
							description={
								<div style={{ justifyContent: 'center', display: 'flex', flexDirection: 'column' }}>
									...
									<div style={{ display: 'flex', justifyContent: 'center' }}>
										<Button className={classes.btn + ' ' + classes.MarginT9} href={'/#/Publish'}>
											{t('去发布')}
										</Button>
									</div>
								</div>
