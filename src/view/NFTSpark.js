...
class NFTSpark extends Component {
	...
	async componentDidMount() {
		...
		if (account === -1) {
			...
			if (currentChainId !== originChainId) {
				...
				alert(t('请切换至')+' ' + originChainName + t('链并查看此NFT'))
		...
		if (token_addr == '0x0000000000000000000000000000000000000000') {
			...
			shill_price_string = shill_price_string.toFormat(3) + ' ' + name;
			...
			try {
				...
				shill_price_string = shill_price_string.toFormat(3) + ' ' + token_symbol;
	...
	downloadIPFS = async () => {
		...
		try {
			...
			let suffix = '.' + this.state.fileType
			let fileName = this.state.Name + suffix
			FileSaver.saveAs(this.NewBlob(response.data,this.state.fileType),fileName)
		} catch (error) {
			message.error({
				content: `Error: ${error}`,
				className: 'custom-class',
