...
class NFTInfo extends Component {
  ...
  async componentDidMount() {
    ...
    const price = await contract.methods
      .getShillPriceByNFTId(this.props.match.params.id)
      .call();
    const owner = await contract.methods
      .ownerOf(this.props.match.params.id)
      .call();
    ...
    if (lastConnect === METAMASK) {
      ...
      if (account.length === 0) {
        await tp
          .getWallet({ walletTypes: ["matic"], switch: false })
          .then((value) => {
            account = value.data.address;
          });
      }
    ...
    const token_addr = await contract.methods
      .getTokenAddrByNFTId(this.props.match.params.id)
      .call();
    const res = await contract.methods
      .getProfitByNFTId(this.props.match.params.id)
      .call();
    ...
    const meta = await contract.methods
      .tokenURI(this.props.match.params.id)
      .call();
    // https://coldcdn.com/api/cdn/v5ynur/ipfs/QmV7s3xrtwxfBa7VDcqHGFKcSjKdLZL7offiC31yM2NSqz
    let hash = meta.split("/");
    const issue = await contract.methods
      .getIssueIdByNFTId(this.props.match.params.id)
      .call();
    ...
    const leafUrl =
      this.backend + "/api/v1/nft/info?nft_id=" + this.props.match.params.id;
    axios
      .get(leafUrl)
      .then((res) => {
      ...
      .catch((error) => {
        ...
        if (
          ...
          error.response.data.message.includes("children not found")
        ) {
  ...
  wordArrayToByteArray = (wordArray, length) => {
    ...
    var result = [],
      ...
      i = 0;
    while (length > 0) {
      var added_info = this.wordToByteArray(
        result,
        wordArray[i],
        Math.min(4, length)
      );
  ...
  render() {
    ...
    const sell_info = () => {
      ...
      let toUrl =
        "https://" + url + "/#/NFT/Spark/" + this.props.match.params.id;
      ...
      return (
        <Spin spinning={this.state.loading} size="large">
          <ThemeProvider theme={theme}>
            <TopBar />
            <div
              style={{ textAlign: "center", marginTop: "2%", marginLeft: 40 }}
            >
              <Typography
                color="textPrimary"
                gutterBottom
                style={{ marginRight: 50, marginTop: 20, fontSize: 24 }}
              >
                <b>正在解密文件</b>
              </Typography>
            </div>
            <div style={{ textAlign: "center", marginTop: "10%" }}>
              <Progress
                type="circle"
                percent={this.state.decryptPercentage}
                width={200}
              />
            </div>
