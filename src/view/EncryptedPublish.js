...
class EncryptedPublish extends Component {
  ...
  jump = async (event) => {
    ...
    var issueId = await contract.methods
      .getIssueIdByNFTId(this.state.rootNFTId)
      .call();
          ...
          try {
            ...
            try {
              ...
              reader.onload = (e) => {
                ...
                var cipher_text = CryptoJS.AES.encrypt(
                  str,
                  secret_key
                ).toString();
              ...
              if (error.response.status == 400) {
                if (error.response.data.message.includes("signature invalid")) {
                  ...
                  error.response.data.message.includes("param invalid")
                ) {
                  message.error({
                    content: "参数错误",
                    className: "custom-class",
...
export default withTranslation()(
  withStyles(styles, { withTheme: true })(EncryptedPublish)
);
//todo 涉及交易
