...
class BuySingle extends Component {
  ...
  async componentDidMount() {
    ...
    const issueId = await contract.methods
      .getIssueIdByNFTId(this.state.NFTId)
      .call();
    const royalty = await contract.methods
      .getRoyaltyFeeByIssueId(issueId)
      .call();
    const metadata = await contract.methods
      .tokenURI(this.props.match.params.NFTId)
      .call();
    ...
    var owner = await contract.methods
      .ownerOf(this.props.match.params.NFTId)
      .call();
    var approved = await contract.methods
      .getApproved(this.props.match.params.NFTId)
      .call();
    var price = await contract.methods
      .getTransferPriceByNFTId(this.props.match.params.NFTId)
      .call();
    var token_addr = await contract.methods
      .getTokenAddrByNFTId(this.props.match.params.NFTId)
      .call();
    this.setState({
      owner: owner,
    ...
    if (token_addr == "0x0000000000000000000000000000000000000000") {
      ...
      try {
        ...
        const approved_amount = await token_contract.methods
          .allowance(account, this.sparkAddr)
          .call();
    ...
    const child_url =
      this.backend + "/api/v1/nft/info?nft_id=" + this.state.NFTId;
    try {
  ...
  render() {
    ...
    const buyButton = () => {
      if (
        this.state.approvedAddr.toLowerCase() !==
        this.state.currentAcc.toLowerCase()
      ) {
        return (
          <Button
            variant="outlined"
              ...
              {this.state.loadItem ? (
                <Grid container spacing={5} className={classes.content}>
                  ...
                  <Grid
                    item
                    xs
                    style={{ marginLeft: "5%", maxWidth: 500, minWidth: 350 }}
                  >
                    <Skeleton
                      animation="wave"
                      variant="text"
                ...
                <Grid container className={classes.content} spacing={5}>
                  ...
                  <Grid item xs className={classes.content2}>
                    ...
                    <Typography
                      ...
                      align="left"
                      noWrap
                      style={{ fontFamily: "Teko", fontSize: 34 }}
                    >
                      <b>{this.state.name}</b>
                    </Typography>
                    <Typography
